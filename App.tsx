...
const BlogCard: React.FC<BlogCardProps> = ({ post, variant = 'grid', onNavigate, allPosts = [] }) => {
        ...
        <div className="lg:w-1/3">
          <div className="bg-[#050119]/40 border border-white/10 rounded-2xl p-6 h-full backdrop-blur-sm">
            ...
            <div className="space-y-8">
              {allPosts.slice(0, 4).map((trending) => (
...
const BlogView = ({ onNavigate }: { onNavigate: (path: string) => void }) => {
  ...
  useEffect(() => {
    if (!loading && slug && !posts.find(p => p.slug === slug)) {
      navigate('/blog', { replace: true });
    }
  }, [loading, slug, posts, navigate]);

  if (loading) {
    return (
      <div className="max-w-7xl mx-auto px-6 pt-16 pb-40 flex items-center justify-center min-h-[60vh]">
        <div className="text-neutral-500 text-lg">Loading articles...</div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 pt-16 pb-40 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      {/* Featured Post */}
      {posts.length > 0 && (
        <BlogCard post={posts[0]} variant="featured" onNavigate={onNavigate} allPosts={posts} />
      )}
